...
async function getDoctorAppointments(req, res) {
    try {
        ...
        const [rows] = await pool.query(
            `SELECT a.id, a.patient_first_name, a.patient_last_name, a.visit_type, a.payment_method, a.payment_status, a.visit_date, a.visit_time
        ...
        const appointments = rows.map(r => ({
            id: r.id,
            doctor: `${r.patient_first_name} ${r.patient_last_name}`,
...
async function getProfile(req, res) {
    try {
        ...
        const [rows] = await pool.query(
            'SELECT id, first_name, last_name, email, phone FROM doctors WHERE id = ?',
            [userId]
        );

        if (rows.length === 0) {
            return res.status(404).json({ message: 'Lekarz nie znaleziony' });
        }
